...
class WillowTvLiveIE(InfoExtractor):
    IE_NAME = 'willowtv:live'
    _VALID_URL = r'https?://(?:www\.)?willow\.tv/watch-live/(?P<id>.*)'
    _TESTS = []

    def _real_extract(self, url):
        meta_format = 'https://willowfeedsv2.willow.tv/willowds/mspecific/%s.json'
        replay_format = 'https://www.willow.tv/match_live_data_by_id?matchid=%s'
        ...
        for result in results:
            ...
            streams = try_get(data, lambda x: x['result'], list) \
                or []
            if not streams:
                raise ExtractorError(
                    'No streams present for this match. '
                    'They may not have been uploaded yet.',
                    expected=True)

            for stream in streams:
                vid_format = try_get(stream, lambda x: x['vidFormat'],
                                     compat_str)
                if vid_format != 'HLS':
                    raise ExtractorError(
                        'Unsupported video format "%s".' % vid_format,
                ...
                part = stream['priority']
                part_id = compat_str(part)
                url = stream['secureurl']

                title = try_get(stream, lambda x: x['title'], compat_str) \
                    or name or slug
